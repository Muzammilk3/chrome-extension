  ...
  switch (request.action) {
    case 'getProductiveSites':
      chrome.storage.local.get(['productiveSites'], (result) => {
        const sites = result.productiveSites || [
          'github.com', 'stackoverflow.com', 'docs.google.com', 
          'notion.so', 'codepen.io', 'developer.mozilla.org'
        ];
        console.log('Sending productive sites:', sites);
        sendResponse(sites);
      });
      return true; // Keep message channel open for async response
      ...
      chrome.storage.local.get(['productiveSites'], (result) => {
        ...
        if (!sites.includes(request.site)) {
          ...
          chrome.storage.local.set({ productiveSites: sites }, () => {
            console.log('Added productive site:', request.site);
            sendResponse({ success: true });
          });
        } else {
    ...
    case 'removeProductiveSite':
      chrome.storage.local.get(['productiveSites'], (result) => {
        const sites = result.productiveSites || [];
        const index = sites.indexOf(request.site);
        if (index > -1) {
          sites.splice(index, 1);
          chrome.storage.local.set({ productiveSites: sites }, () => {
            console.log('Removed productive site:', request.site);
            sendResponse({ success: true });
          });
        } else {
