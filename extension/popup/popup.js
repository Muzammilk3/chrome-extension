...
class ProductivityPopup {
  ...
  showActiveFocusMode(focusData) {
    ...
    const timerValue = document.getElementById('focusTimerValue');
    if (focusControls) focusControls.classList.remove('hidden');
    if (focusStatusText) focusStatusText.textContent = 'Focus mode active - Sites are blocked';
    if (startBtn) startBtn.style.display = 'none';
    if (stopBtn) stopBtn.style.display = 'block';
    if (timerDisplay) timerDisplay.style.display = 'block';
    if (focusData && focusData.endTime) {
      this.startFocusCountdown(focusData.endTime);
    }
  }
  ...
  async startFocusMode() {
    ...
    try {
      ...
      // Show active focus mode with timer
      this.showActiveFocusMode({ active: true, endTime });
      // Also reload focus data to get the actual backend state
      setTimeout(() => {
        this.loadFocusData();
      }, 500);
    } catch (error) {
  ...
  startFocusCountdown(endTime) {
    ...
    update();
    this._focusTimerInterval = setInterval(update, 1000);
  }
  ...
  async addBlockedSite() {
    ...
    if (!this.isValidDomain(cleanSite)) {
      this.showError('Please enter a valid domain with proper extension (e.g., facebook.com, youtube.org)');
      return;
    }
    // Check for duplicate
    let blockedSites = [];
    try {
      blockedSites = await chrome.runtime.sendMessage({ action: 'getBlockedSites' });
    } catch (e) {}
    if (blockedSites && blockedSites.includes(cleanSite)) {
      this.showError('Site is already added');
      return;
    }
  ...
  async addFocusBlockedSite() {
    ...
    // Validate domain format
    if (!this.isValidDomain(cleanSite)) {
      this.showError('Please enter a valid domain with proper extension (e.g., youtube.com, reddit.org)');
      return;
    }
    // Check for duplicate
    let focusSites = [];
    try {
      focusSites = await chrome.runtime.sendMessage({ action: 'getFocusBlockedSites' });
    } catch (e) {}
    if (focusSites && focusSites.includes(cleanSite)) {
      this.showError('Site is already added');
      return;
    }
  ...
  isValidDomain(domain) {
    ...
    // Check for basic structure (must contain at least one dot)
    if (!cleanDomain.includes('.')) return false;
    
    // More strict domain validation
    // Must have at least 2 characters before the TLD and at least 2 characters for TLD
    const domainRegex = /^[a-zA-Z0-9][a-zA-Z0-9\-]{1,}[a-zA-Z0-9]\.[a-zA-Z]{2,}$/;
    
    // Additional checks
    const parts = cleanDomain.split('.');
    if (parts.length < 2) return false;
    
    // Check if the domain part (before TLD) is at least 2 characters
    const domainPart = parts[0];
    const tld = parts[parts.length - 1];
    
    // Stricter validation: domain part must be at least 3 characters, TLD at least 2
    if (domainPart.length < 3 || tld.length < 2) return false;
    
    // Ensure TLD contains only letters (no numbers)
    if (!/^[a-zA-Z]+$/.test(tld)) return false;
    
    // Check against known valid TLDs (expanded list)
    const validTlds = [
      // Generic TLDs
      'com', 'org', 'net', 'edu', 'gov', 'mil', 'int', 'info', 'biz', 'name', 'pro',
      // New generic TLDs
      'io', 'dev', 'app', 'tech', 'online', 'website', 'site', 'blog', 'news', 'shop',
      // Country code TLDs
      'uk', 'us', 'ca', 'au', 'de', 'fr', 'jp', 'cn', 'in', 'br', 'mx', 'ru', 'it', 'es', 'nl'
    ];
    
    // If TLD is in our known list, allow it
    if (validTlds.includes(tld.toLowerCase())) {
      return domainRegex.test(cleanDomain);
    }
    
    // For unknown TLDs, only allow if they are 2+ letters and pass regex
  ...
  async addProductiveSite() {
    ...
    // Validate domain format
    if (!this.isValidDomain(cleanSite)) {
      this.showError('Please enter a valid domain with proper extension (e.g., github.com, stackoverflow.org)');
      return;
    }
...
// --- Ad Blocker Knob (Switch) Logic with ON/OFF text ---
document.addEventListener('DOMContentLoaded', () => {
  const adBlockerKnob = document.getElementById('adBlockerKnob');
  const adBlockerStatus = document.getElementById('adBlockerStatus');
  const adBlockerStatusText = document.getElementById('adBlockerStatusText');

  function updateKnobAndStatus(enabled) {
    if (adBlockerKnob) adBlockerKnob.checked = !!enabled;
    if (adBlockerStatusText) {
      adBlockerStatusText.textContent = enabled ? 'ON' : 'OFF';
      adBlockerStatusText.style.color = enabled ? '#000000ff' : '#000000ff';
    }
    if (adBlockerStatus) {
      adBlockerStatus.textContent = enabled ? 'Ad Blocker is enabled and blocking common ad domains.' : 'Ad Blocker is disabled.';
      adBlockerStatus.style.color = enabled ? '#2e7d32' : '#000000ff';
    }
  }

  // Load state from chrome.storage
  chrome.storage.local.get(['adBlockerEnabled'], (result) => {
    updateKnobAndStatus(!!result.adBlockerEnabled);
  });

  // Listen for knob (switch) changes
  if (adBlockerKnob) {
    adBlockerKnob.addEventListener('change', (e) => {
      const enabled = e.target.checked;
      chrome.storage.local.set({ adBlockerEnabled: enabled }, () => {
        chrome.runtime.sendMessage({ action: 'toggleAdBlocker', enabled });
        updateKnobAndStatus(enabled);
      });
    });
  }

  // Listen for storage changes (sync knob if changed elsewhere)
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes.adBlockerEnabled) {
      updateKnobAndStatus(!!changes.adBlockerEnabled.newValue);
    }
  });
});
